...
@section('body_class', 'bg-slate-50 dark:bg-slate-950 overflow-hidden h-screen flex')

@section('layout')
    
    <!-- Sidebar -->
    <aside class="w-64 min-w-[16rem] max-w-[16rem] flex-none bg-white dark:bg-slate-900 border-r border-gray-200 dark:border-slate-800 flex flex-col transition-all duration-300 z-20">
        <!-- Logo -->
        ...
        <nav class="flex-1 overflow-y-auto no-scrollbar py-4 px-3 space-y-1">
            ...
            <a href="{{ route('categories.index') ?? '#' }}" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-xl group transition-colors {{ request()->routeIs('categories.*') ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400' : 'hover:bg-indigo-50 dark:hover:bg-indigo-500/10 text-gray-700 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400' }}">
                <svg class="w-5 h-5 mr-3 {{ request()->routeIs('categories.*') ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 group-hover:text-indigo-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path></svg>
        ...
        <div class="p-4 border-t border-gray-200 dark:border-slate-800 relative" x-data="{ open: false }" @click.away="open = false">
            <button @click="open = !open" class="flex items-center w-full px-2 py-2 text-sm rounded-xl hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors cursor-pointer text-left focus:outline-none">
    ...
    <!-- Main wrapper -->
    <div class="flex-1 flex flex-col min-w-0 w-full overflow-hidden bg-slate-50 dark:bg-slate-950 relative">
        ...
        <!-- Page Content -->
        <main class="flex-1 overflow-y-auto overflow-x-hidden p-6 z-10">
